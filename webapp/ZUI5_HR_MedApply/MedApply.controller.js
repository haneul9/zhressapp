...
sap.ui.define([
	...
	function (Common, CommonController, JSONModelHelper, PageHelper,EmployeeModel,HoverIcon,InputBase,BusyIndicator,OrgOfIndividualHandler,DialogHandler) {
	"use strict";
	...
	//					MedicalApplyTableInH:[]
		...
		onBeforeShow: function(oEvent){
			...
			oController._vPernr=oSessionData.Pernr;
			if($.app.APP_AUTH=="H"){
				$.app.byId(oController.PAGEID+"_HassPer").setVisible(true);
				$.app.byId(oController.PAGEID+"_HassPer").getItems()[1].removeAllCustomData();
				$.app.byId(oController.PAGEID+"_HassPer").getItems()[1].addCustomData(new sap.ui.core.CustomData({
					key : "Pernr",
					value : oController._vPernr
				}));
				oController._Hass="X";
			}else{
				$.app.byId(oController.PAGEID+"_HassPer").setVisible(false);			
			}

			$.app.byId(oController.PAGEID + "_HeadSel").addItem();
			var oModel=$.app.getModel("ZHR_BENEFIT_SRV");
			oModel.create("/MedicalBukrsImportSet", {Pernr:oController._vPernr,Datum:new Date(),MedicalBukrsExport:[]},
					{success:function(data,res){
...
//			common.EmpBasicInfoBoxCustom.setHeader(this._SessionData.Pernr);
		...
		getBukrs : function(vDatum){
			...
			var oSessionData=oController._SessionData;
			function getBukrs(){
				oModel.create("/MedicalBukrsImportSet", {Pernr:oController._vPernr,
		...
		getSelData2: function(vSig){
			...
			var oSessionData=oController._SessionData;	
			var oModel=$.app.getModel("ZHR_COMMON_SRV");
			var vData={ICodeT:"001",ICodty:"GTZ51",IBukrs:oController._Bukrs,IPernr:oController._vPernr,NavCommonCodeList:[],ILangu:"3"};				
		...
		onDialog : function(New,Flag){
			...
			if(Flag=="1000"){
				...
				if(oController._Hass=="X"){
					oController._vPernr=$.app.byId(oController.PAGEID+"_HassPer").getItems()[1].getCustomData()[0].getValue("Pernr");
				}
		...
		onChange3: function(vSig){
			...
			var oSel4=$.app.byId(oController.PAGEID+"_dSel4");
			var oSessionData=oController._SessionData;	
			var vData={ICodeT:"002",ICodty:"GTZ51",IBukrs:oController._Bukrs,IPernr:oController._vPernr,ICode:oSel3.getSelectedKey(),NavCommonCodeList:[],ILangu:"3"};	
			var oModel=$.app.getModel("ZHR_COMMON_SRV");

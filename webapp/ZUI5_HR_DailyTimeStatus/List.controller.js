...
sap.ui.define([
	...
	return CommonController.extend("ZUI5_HR_DailyTimeStatus.List", {
		...
		onPressSearch : function(oEvent){
			...
			if(!oData.Tmdat){
				sap.m.MessageBox.error(oBundleText.getText("MSG_43001")); // 조회일자를 입력하여 주십시오.
				return;
			} else if(!oData.Werks){
				sap.m.MessageBox.error(oBundleText.getText("MSG_43002")); // 사업장을 선택하여 주십시오.
				return;
			}
			...
			var search = function(){
				...
				var createData1 = {TimeGroupStatNav : []};
					...
					createData1.ITmdat = "\/Date(" + common.Common.getTime(new Date(oData.Tmdat)) + ")\/";
					createData1.ILangu = oData.Langu;
					createData1.IEmpid = oData.Pernr;
					createData1.IAusty = gAuth;
				...
				oModel.create("/TimeGroupStatusSet", createData1, null,
					function(data, res){
						if(data){
				...
				var createData2 = {WeeklyWorkingTimeNav : []};
					...
					createData2.ITmdat = "\/Date(" + common.Common.getTime(new Date(oData.Tmdat)) + ")\/";
					createData2.ILangu = oData.Langu;
					createData2.IEmpid = oData.Pernr;
					createData2.IAusty = gAuth;
				...
				oModel.create("/WeeklyWorkingTimeStatusSet", createData2, null,
					function(data, res){
						if(data){
				...
				var createData3 = {OvertimeStatNav : []};
					...
					createData3.ITmdat = "\/Date(" + common.Common.getTime(new Date(oData.Tmdat)) + ")\/";
					createData3.ILangu = oData.Langu;
					createData3.IEmpid = oData.Pernr;
					createData3.IAusty = gAuth;
				...
				oModel.create("/OvertimeStatusSet", createData3, null,
					function(data, res){
						if(data){
		...
		onPressDetail : function(oEvent){
			...
			if(oFlcty == "Empcnt"){ // 인원수
				oFlcty = "A";
			} else if(oFlcty == "Attcnt"){ // 출근
				oFlcty = "B";
			} else {
			...
			var createData = {TimeGroupStatDetailNav : []};
				...
				createData.ITmdat = "\/Date(" + common.Common.getTime(new Date(oFilter.Tmdat)) + ")\/";
			...
			oModel.create("/TimeGroupStatusDetailSet", createData, null,
				function(data, res){
					if(data){
						if(data.TimeGroupStatDetailNav && data.TimeGroupStatDetailNav.results){
							for(var i=0; i<data.TimeGroupStatDetailNav.results.length; i++){
								...
								data.TimeGroupStatDetailNav.results[i].Beguz = data.TimeGroupStatDetailNav.results[i].Beguz == "" ? "" : 
									data.TimeGroupStatDetailNav.results[i].Beguz.substring(0,2) + ":" + data.TimeGroupStatDetailNav.results[i].Beguz.substring(2,4);
								data.TimeGroupStatDetailNav.results[i].Enduz = data.TimeGroupStatDetailNav.results[i].Enduz == "" ? "" : 
									data.TimeGroupStatDetailNav.results[i].Enduz.substring(0,2) + ":" + data.TimeGroupStatDetailNav.results[i].Enduz.substring(2,4);
									
								data.TimeGroupStatDetailNav.results[i].Stdaz = parseFloat(data.TimeGroupStatDetailNav.results[i].Stdaz) == 0 ? 
									"" : data.TimeGroupStatDetailNav.results[i].Stdaz;
		...
		onPressOvertimeStatus : function(oEvent){
			...
			oModel.create("/OvertimeStatusDetailSet", createData, null,
				function(data, res){
					if(data){
		...
		onExport : function(oEvent){
			...
			var filename = oBundleText.getText("LABEL_43045"); // 개인별 근무시간현황

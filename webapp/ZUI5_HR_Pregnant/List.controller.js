...
sap.ui.define([
	...
	function (Common, CommonController, JSONModelHelper, PageHelper, AttachFileAction) {
	"use strict";
		...
		onChangeDate : function(oEvent, field){
			...
			switch(field){
				case "Preen": // 출산(예정)일 - 저장된 데이터 여부 확인
					...
					var detail = {Pernr : vEmpLoginInfo.Pernr, Preen : "\/Date(" + common.Common.getTime(new Date(oData.Preen)) + ")\/"};
					if(oData.Status1 == "AA"){
						detail.Subty = oData.Subty;
						detail.Objps = oData.Objps;
						detail.Sprps = oData.Sprps;
						detail.Begda = "\/Date(" + common.Common.getTime(oData.Begda) + ")\/";
						detail.Endda = "\/Date(" + common.Common.getTime(oData.Endda) + ")\/";
				...
				case "Prebg": // 임신시작일
					...
					var detail = {Pernr : vEmpLoginInfo.Pernr, Preen : "\/Date(" + common.Common.getTime(new Date(oData.Preen)) + ")\/", Prebg : "\/Date(" + common.Common.getTime(new Date(oData.Prebg)) + ")\/"};
					if(oData.Status1 == "AA"){
						detail.Subty = oData.Subty;
						detail.Objps = oData.Objps;
						detail.Sprps = oData.Sprps;
				...
				default:
					...
					if(value != ""){
						...
						if(value < (dateFormat.format(new Date()) * 1)){
							sap.m.MessageBox.error(oBundleText.getText("MSG_39019")); // 현재일 이전 일자는 선택 불가능합니다.
							oEvent.getSource().setValue("");
						}
					}
			}
			...
			if(oField.length != 0){
				oModel.create("/PregnantApplyHeaderSet", createData, null,
					function(data, res){
						if(data){
							...
							if(data.PregnantApplyTableIn && data.PregnantApplyTableIn.results){
								...
								// 출산예정일, 임신시작일
								datas.Preen = datas.Preen ? dateFormat.format(new Date(common.Common.setTime(datas.Preen))) : "";
								datas.Prebg = datas.Prebg ? dateFormat.format(new Date(common.Common.setTime(datas.Prebg))) : "";
								
								// 단축근무기간
								datas.Begsh = datas.Begsh ? dateFormat.format(new Date(common.Common.setTime(datas.Begsh))) : "";
								datas.Endsh = datas.Endsh ? dateFormat.format(new Date(common.Common.setTime(datas.Endsh))) : "";
								datas.Begsh2 = datas.Begsh2 ? dateFormat.format(new Date(common.Common.setTime(datas.Begsh2))) : "";
								datas.Endsh2 = datas.Endsh2 ? dateFormat.format(new Date(common.Common.setTime(datas.Endsh2))) : "";
								
								// 신청가능기간
								datas.Begshp = datas.Begshp ? dateFormat.format(new Date(common.Common.setTime(datas.Begshp))) : "";
								datas.Begsh2p = datas.Begsh2p ? dateFormat.format(new Date(common.Common.setTime(datas.Begsh2p))) : "";
								datas.Endshp = datas.Endshp ? dateFormat.format(new Date(common.Common.setTime(datas.Endshp))) : "";
								datas.Endsh2p = datas.Endsh2p ? dateFormat.format(new Date(common.Common.setTime(datas.Endsh2p))) : "";
								
								// 태아수 없는 경우 기본값 1로 세팅
								datas.Prebn = datas.Prebn == "0" ? "1" : datas.Prebn;
								
								for(var i=0; i<oField.length; i++){
									eval("oController._ListCondJSonModel.setProperty('/Data/" + oField[i] + "', datas." + oField[i] + ");");
								}
							}
						}
					},
				...
				if(oController._ListCondJSonModel.getProperty("/Data/Eretcode") == "X"){
					sap.m.MessageBox.information(oController._ListCondJSonModel.getProperty("/Data/Erettext"));
				} else if(oController._ListCondJSonModel.getProperty("/Data/Eretcode") == "Z"){
					sap.m.MessageBox.error(oController._ListCondJSonModel.getProperty("/Data/Erettext"));
				}
			...
			if(oController._ListCondJSonModel.getProperty("/Data/Eretcode") == "X"){
				sap.ui.getCore().byId(oController.PAGEID + "_FileRow").addStyleClass("displayNone");
			} else {
				sap.ui.getCore().byId(oController.PAGEID + "_FileRow").removeStyleClass("displayNone");
			}
		},
		
		// 출산(예정)일이 현재일 이전이면 임신시작일, 단축근무시간, 단축근무기간 입력 불가
		onCheckPreen : function(oEvent){
			var oView = sap.ui.getCore().byId("ZUI5_HR_Pregnant.List");
			var oController = oView.getController();
						...
						if(data){
							if(data.PregnantApplyTableIn && data.PregnantApplyTableIn.results){
								for(var i=0; i<data.PregnantApplyTableIn.results.length; i++){
									...
									data.PregnantApplyTableIn.results[i].Begda = data.PregnantApplyTableIn.results[i].Begda ? new Date(common.Common.setTime(data.PregnantApplyTableIn.results[i].Begda)) : null;
									data.PregnantApplyTableIn.results[i].Endda = data.PregnantApplyTableIn.results[i].Endda ? new Date(common.Common.setTime(data.PregnantApplyTableIn.results[i].Endda)) : null;
									
									data.PregnantApplyTableIn.results[i].Prebg = data.PregnantApplyTableIn.results[i].Prebg ? new Date(common.Common.setTime(data.PregnantApplyTableIn.results[i].Prebg)) : null;
									data.PregnantApplyTableIn.results[i].Preen = data.PregnantApplyTableIn.results[i].Preen ? new Date(common.Common.setTime(data.PregnantApplyTableIn.results[i].Preen)) : null;
									
									// 단축근무기간 12주이내
									data.PregnantApplyTableIn.results[i].Begsh = data.PregnantApplyTableIn.results[i].Begsh ? new Date(common.Common.setTime(data.PregnantApplyTableIn.results[i].Begsh)) : null;
									data.PregnantApplyTableIn.results[i].Endsh = data.PregnantApplyTableIn.results[i].Endsh ? new Date(common.Common.setTime(data.PregnantApplyTableIn.results[i].Endsh)) : null;
									// 신청가능기간
									data.PregnantApplyTableIn.results[i].Begshp = data.PregnantApplyTableIn.results[i].Begshp ? new Date(common.Common.setTime(data.PregnantApplyTableIn.results[i].Begshp)) : null;
									data.PregnantApplyTableIn.results[i].Endshp = data.PregnantApplyTableIn.results[i].Endshp ? new Date(common.Common.setTime(data.PregnantApplyTableIn.results[i].Endshp)) : null;
									
									// 단축근무기간 36주이후
									data.PregnantApplyTableIn.results[i].Begsh2 = data.PregnantApplyTableIn.results[i].Begsh2 ? new Date(common.Common.setTime(data.PregnantApplyTableIn.results[i].Begsh2)) : null;
									data.PregnantApplyTableIn.results[i].Endsh2 = data.PregnantApplyTableIn.results[i].Endsh2 ? new Date(common.Common.setTime(data.PregnantApplyTableIn.results[i].Endsh2)) : null;
									// 신청가능기간
									data.PregnantApplyTableIn.results[i].Begsh2p = data.PregnantApplyTableIn.results[i].Begsh2p ? new Date(common.Common.setTime(data.PregnantApplyTableIn.results[i].Begsh2p)) : null;
									data.PregnantApplyTableIn.results[i].Endsh2p = data.PregnantApplyTableIn.results[i].Endsh2p ? new Date(common.Common.setTime(data.PregnantApplyTableIn.results[i].Endsh2p)) : null;
    ...
    //             });
		...
		onPressSearch2 : function(oEvent){
			...
			var createData = {NavCommonCodeList : []};
				...
				createData.IDatum = vData.Data.Reqdt ? "\/Date(" + common.Common.getTime(new Date(vData.Data.Reqdt)) + ")\/" : "\/Date(" + common.Common.getTime(new Date()) + ")\/";
		...
		onPressSave : function(oEvent, Flag){
			...
			if(Flag != "D"){
				// validation check
				if(oData.Eretcode == "Z"){
					sap.m.MessageBox.error(oData.Erettext);
					return;
				} 
				
				if(!oData.Preen || oData.Preen == ""){
					sap.m.MessageBox.error(oBundleText.getText("MSG_39015")); // 출산(예정)일을 입력하여 주십시오.
					return;
				} else if((!oData.EditFalse || oData.EditFalse == "") && (!oData.Prebg || oData.Prebg == "")){
					sap.m.MessageBox.error(oBundleText.getText("MSG_39016")); // 임신시작일을 입력하여 주십시오.
					return;
				} else if(oData.Prebn == "" || oData.Prebn == "0"){
					sap.m.MessageBox.error(oBundleText.getText("MSG_39017")); // 태아수를 입력하여 주십시오.
					return;
				} else if(!oData.EditFalse || oData.EditFalse == ""){
					if(!oData.Pampm || oData.Pampm == ""){
						sap.m.MessageBox.error(oBundleText.getText("MSG_39018")); // 단축근무시간을 선택하여 주십시오.
						return;
					} else if(oData.Pampm != "9"){ // 단축근무 제외를 선택하지 않은 경우, 단축근무기간 중 하나는 입력되어야 한다.
						if(((oData.Begsh && !oData.Endsh) || (!oData.Begsh && oData.Endsh)) || ((oData.Begsh2 && !oData.Endsh2) || (!oData.Begsh2 && oData.Endsh2))){
							sap.m.MessageBox.error(oBundleText.getText("MSG_39020")); // 단축근무기간을 입력하여 주십시오.
							return;
							...
							if(oData.Begsh && oData.Endsh){
								if((check(oData.Begsh, oData.Begshp, "1") == false) || (check(oData.Endsh, oData.Endshp, "2") == false)){
									sap.m.MessageBox.error(oBundleText.getText("MSG_39022")); // 단축근무기간(12주이내) 값이 임신주수기준의 기간을 벗어났습니다. 확인하시기 바랍니다.
									return;
								}
							}
							...
							if(oData.Begsh2 && oData.Endsh2){
								if((check(oData.Begsh2, oData.Begsh2p, "1") == false) || (check(oData.Endsh2, oData.Endsh2p, "2") == false)){
									sap.m.MessageBox.error(oBundleText.getText("MSG_39023")); // 단축근무기간(36주이후) 값이 임신주수기준의 기간을 벗어났습니다. 확인하시기 바랍니다.
									return;
								}
							}
						}
				...
				if(oData.Eretcode == "X"){
					...
					if(AttachFileAction.getFileLength(oController) === 0) {
						sap.m.MessageBox.error(oBundleText.getText("MSG_39021"), { // 첨부파일을 업로드 해주세요.
							title : oBundleText.getText("LABEL_00149") // 안내
						});
						return;
					};
				}
			...
			if(Flag == "S"){
				...
				confirmMessage = oBundleText.getText("MSG_39013"); // 삭제하시겠습니까?
				successMessage = oBundleText.getText("MSG_39014"); // 삭제되었습니다.
			}
			...
			var onProcess = function(){
				...
				var createData = {PregnantApplyTableIn : []};
					...
					createData.ILangu = $.app.getModel("session").getData().Langu;
					createData.IBegda = oData.Begda ? "\/Date(" + common.Common.getTime(oData.Begda) + ")\/" : null;
					createData.IEndda = oData.Endda ? "\/Date(" + common.Common.getTime(oData.Endda) + ")\/" : null;
				
					var detail = {};
						detail.Pernr = $.app.getModel("session").getData().Pernr;
						detail.Preen = "\/Date(" + common.Common.getTime(new Date(oData.Preen)) + ")\/"; // 출산예정일
						detail.Prebg = "\/Date(" + common.Common.getTime(new Date(oData.Prebg)) + ")\/"; // 임신시작일
						detail.Prebn = oData.Prebn; // 태아수
						detail.Pampm = oData.Pampm; // 단축근무시간
						detail.Mptyp = oData.Mptyp ? oData.Mptyp : ""; // 법정관리유형
						
						// 단축근무기간
						detail.Begsh = oData.Begsh ? "\/Date(" + common.Common.getTime(new Date(oData.Begsh)) + ")\/" : null;
						detail.Endsh = oData.Endsh ? "\/Date(" + common.Common.getTime(new Date(oData.Endsh)) + ")\/" : null;
						detail.Begshp = oData.Begshp ? "\/Date(" + common.Common.getTime(new Date(oData.Begshp)) + ")\/" : null;
						detail.Endshp = oData.Endshp ? "\/Date(" + common.Common.getTime(new Date(oData.Endshp)) + ")\/" : null;
						
						// 신청가능기간
						detail.Begsh2 = oData.Begsh2 ? "\/Date(" + common.Common.getTime(new Date(oData.Begsh2)) + ")\/" : null;
						detail.Endsh2 = oData.Endsh2 ? "\/Date(" + common.Common.getTime(new Date(oData.Endsh2)) + ")\/" : null;
						detail.Begsh2p = oData.Begsh2p ? "\/Date(" + common.Common.getTime(new Date(oData.Begsh2p)) + ")\/" : null;
						detail.Endsh2p = oData.Endsh2p ? "\/Date(" + common.Common.getTime(new Date(oData.Endsh2p)) + ")\/" : null;
						
						detail.Reque = oData.Reque; // 요청사항
						
						if(oData.Reqdt){
							detail.Reqdt = "\/Date(" + common.Common.getTime(oData.Reqdt) + ")\/"; // 신청일자
						} else {
							detail.Reqdt = "\/Date(" + common.Common.getTime(new Date()) + ")\/"; // 신청일자
						}
						...
						detail.Begda = oData.Begda ? "\/Date(" + common.Common.getTime(oData.Begda) + ")\/" : null;
						detail.Endda = oData.Endda ? "\/Date(" + common.Common.getTime(oData.Endda) + ")\/" : null;

...
ui: function() {
	...
	return [
		'<div class="card portlet portlet-${size}h portlet-notice" data-key="${key}"${tooltip}>'.interpolate(this.size(), this.key(), this.tooltip()),
				...
				return [
					'<a href="#" class="list-group-item list-group-item-action"${url}>'.interpolate(this.itemUrl(o)),
						'<div class="portlet-notice-item">',
							'<div class="portlet-notice-title">',
								'<span class="portlet-notice-title-text" title="${title}">${title}</span>'.interpolate(o.Title, o.Title),
								o.Newitem === 'X' ? '<span class="badge badge-primary badge-pill">N</span>' : '',
								o.Impor === 'X' ? '<i class="fas fa-exclamation-circle"></i>' : '',
							'</div>',
							'<small class="portlet-notice-date">${date}</small>'.interpolate(date),
						'</div>',
					'</a>'
				].join('');
			}.bind(this)).join(''));

...
sap.ui.define(
	...
	function (Common, SearchUser1, SearchOrg, DialogHandler, OrgOfIndividualHandler, WorkSchedule, ODataService, MessageBox, BusyIndicator,
		Spreadsheet, JSONModel) {
		"use strict";
		...
		var Handler = {
			...
			load: function () {
				...
				this.oModel.setProperty(
					"/Tprogs", [{
						Code: "",
						Text: this.oController.getBundleText("LABEL_00193")
					}].concat( // -- 선택 --
						ODataService.CommonCodeListHeaderSet.call(this.oController, {
							IsContainsAll: false,
				...
				this.oModel.setProperty(
					...
					results.Worktimetab1.map(function (elem) {
						return $.extend(true, elem, {
							PlanTime: elem.Beguz ? "${start} ~ ${End}".interpolate(moment(elem.Beguz.ms).subtract(9, "hours").format("HH:mm"), moment(
								elem.Enduz.ms).subtract(9, "hours").format("HH:mm")) : "",
							InoutTime: elem.Enttm ? "${start} ~ ${End}".interpolate(moment(elem.Enttm.ms).subtract(9, "hours").format("HH:mm"), moment(
								elem.Outtm.ms).subtract(9, "hours").format("HH:mm")) : "",
							WorkTime: elem.Wkbuz ? "${start} ~ ${End}".interpolate(moment(elem.Wkbuz.ms).subtract(9, "hours").format("HH:mm"), moment(
								elem.Wkeuz.ms).subtract(9, "hours").format("HH:mm")) : "",
							AddTime1: elem.Trbuz ? "${start} ~ ${End}".interpolate(moment(elem.Trbuz.ms).subtract(9, "hours").format("HH:mm"), moment(
								elem.Treuz.ms).subtract(9, "hours").format("HH:mm")) : "",
							AddTime2: elem.Trbu1 ? "${start} ~ ${End}".interpolate(moment(elem.Trbu1.ms).subtract(9, "hours").format("HH:mm"), moment(
								elem.Treu1.ms).subtract(9, "hours").format("HH:mm")) : ""
						});
					})
				);
			...
			pressExcelDownloadBtn: function () {
				...
				new Spreadsheet({
					...
					fileName: "${fileName}-${datetime}.xlsx".interpolate(this.oController.getBundleText("LABEL_55039"), sap.ui.core.format.DateFormat
						.getDateTimeInstance({
							pattern: "yyyy-MM-dd"
						}).format(new Date()))
				}).build();
			...
			pressOpenApprovalBtn: function () {
				...
				this.oModel.setProperty("/Detail", {
					...
					Header: {
						...
						WeekName: this.oController.getBundleText(this.oModel.getProperty("/Weeks")[moment().subtract(1, "days").day()]),
						List: [{
							Tim00: "",
							Tim01: "",
							Tim07: "",
			...
			pressOpenApprovalCancelBtn: function () {
				var oTable = $.app.byViewId("PostTable"),
					sIndexes = $.app.byViewId("PostTable").getSelectedIndices(),
					vDeleteTargetAppnms = [],
				...
				sIndexes.forEach(function (sIndex) {
					var rowData = oTable.getModel().getProperty("/List")[sIndex];
			...
			changeTprog: function (oEvent) {
				...
				this.oModel.getProperty("/Tprogs").some(function (elem) {
					if (elem.Code === oControl.getSelectedKey()) {
						...
						this.oModel.setProperty("/Detail/Header/Beguzc", elem.Beguz.ms ? moment(elem.Beguz.ms).subtract(9, "hours").format("HH:mm") :
							"00:00");
						this.oModel.setProperty("/Detail/Header/Enduz", elem.Enduz);
						this.oModel.setProperty("/Detail/Header/Enduzc", elem.Enduz.ms ? moment(elem.Enduz.ms).subtract(9, "hours").format("HH:mm") :
							"00:00");
			...
			searchDetailData: function (arg) {
				...
				this.oModel.setProperty(
					"/Detail/List",
					results.Worktimetab2.length ? results.Worktimetab2.map(function (elem) {
						return $.extend(true, elem, {
			...
			validControl: function (oControl) {
				...
				childItems.forEach(function (control) {
					try {
						...
						if (Object.keys(WorkSchedule.ValidateProperties).indexOf(constructorName) > -1 && control.getRequired() && control.getProperty(
								WorkSchedule.ValidateProperties[constructorName]) === "") {
				...
				var Process = function (fVal) {
					...
					payload.Worktimetab1 = [
						$.extend(true, Common.copyByMetadata(oModel, "WorktimeApplyTab1", oInputData.Header), {
							...
							Wkbuzc: oInputData.Header.WkbuzT && oInputData.Header.WkbuzM ? oInputData.Header.WkbuzT + oInputData.Header.WkbuzM : null,
							Wkeuzc: oInputData.Header.WkeuzT && oInputData.Header.WkeuzM ? oInputData.Header.WkeuzT + oInputData.Header.WkeuzM : null,
							Trbuzc: oInputData.Header.TrbuzT && oInputData.Header.TrbuzM && oInputData.Header.TreuzT && oInputData.Header.TreuzM ?
								oInputData.Header.TrbuzT + oInputData.Header.TrbuzM : null,
							Treuzc: oInputData.Header.TrbuzT && oInputData.Header.TrbuzM && oInputData.Header.TreuzT && oInputData.Header.TreuzM ?
								oInputData.Header.TreuzT + oInputData.Header.TreuzM : null,
							Trbu1c: oInputData.Header.Trbu1T && oInputData.Header.Trbu1M && oInputData.Header.Treu1T && oInputData.Header.Treu1M ?
								oInputData.Header.Trbu1T + oInputData.Header.Trbu1M : null,
							Treu1c: oInputData.Header.Trbu1T && oInputData.Header.Trbu1M && oInputData.Header.Treu1T && oInputData.Header.Treu1M ?
								oInputData.Header.Treu1T + oInputData.Header.Treu1M : null
						})
					];
					payload.Worktimetab2 = oInputData.List.map(function (elem) {
						return Common.copyByMetadata(oModel, "WorktimeApplyTab2", elem);

...
], function(Common, CommonController, momentjs, 
	JSONModelHelper,
	UriParameters, JSONModel) {
"use strict";
...
return CommonController.extend($.app.APP_ID, { //
	...
	onBeforeShow: function() {
		...
		curDate = new Date(),
		vYear = [{ "Code" : "" + curDate.getFullYear(), "Text" : curDate.getFullYear()},
		...
		oController._DetailJSonModel.setProperty("/Data", {  Year :  "" + curDate.getFullYear(), 
	...
	onPressSearchDetail : function(){
		...
		if(!vCondiData.Seqnr ||  vCondiData.Seqnr == ""){
			// 조회조건을 모두 입력하시기 바랍니다.
			sap.m.MessageBox.error(oController.getBundleText("MSG_54001"));
			return;
		}
		
		var search = function(){
			var oPath = "";
			var createData = {PayslipForm1Nav : [],
								PayslipForm2Nav : [],
								PayslipForm3Nav : [],
								PayslipForm4Nav : []
			};
					...
					if(data){
						...
						oMoney4Layout.addItem(
							new sap.m.HBox({
								...
								alignItems: sap.m.FlexAlignItems.Center,
								items: [
									new sap.m.Label({ width: "100%", text: oController.getBundleText("LABEL_54016"), textAlign : "Center" }).addStyleClass("sub-title"),
								]
							})
							.addStyleClass("sub-con-titleBar-both")							
							.setModel(oController._DetailJSonModel)
						...
						if(data.PayslipForm1Nav && data.PayslipForm1Nav.results.length > 0){
							...
							for(var i = 0; i <data.PayslipForm1Nav.results.length; i++ ){
								if(data.PayslipForm1Nav.results[i].Pyitm != ""){
									vData.money1 += (common.Common.toNumber(data.PayslipForm2Nav.results[i].BetrgT) * 1);
									oMoney1Layout.addItem(
										new sap.m.HBox({
											height: "40px",
											alignItems: sap.m.FlexAlignItems.Center,
						...
						if(data.PayslipForm2Nav && data.PayslipForm2Nav.results.length > 0){
							vData.money2  = 0;
							for(var i = 0; i <data.PayslipForm2Nav.results.length; i++ ){
								if(data.PayslipForm2Nav.results[i].Pyitm != ""){
									vData.money2 += (common.Common.toNumber(data.PayslipForm2Nav.results[i].BetrgT) * 1);
									oMoney2Layout.addItem(
										new sap.m.HBox({
											height: "40px",
											alignItems: sap.m.FlexAlignItems.Center,
											items: [
												new sap.m.Label({ width: "150px", text: data.PayslipForm2Nav.results[i].Pyitx, textAlign : "Left" }).addStyleClass("sub-conRead-title"),
												new sap.m.Label({
													textAlign : "End",
													width: "100%",
													layoutData: new sap.m.FlexItemData({ growFactor: 1 }),
						...
						if(data.PayslipForm3Nav && data.PayslipForm3Nav.results.length > 0){
							vData.money3  = 0;
							for(var i = 0; i <data.PayslipForm3Nav.results.length; i++ ){
								if(data.PayslipForm3Nav.results[i].Pyitm != ""){
									vData.money3 += (common.Common.toNumber(data.PayslipForm3Nav.results[i].BetrgT) * 1);
									oMoney3Layout.addItem(
										new sap.m.HBox({
											height: "40px",
											alignItems: sap.m.FlexAlignItems.Center,
											items: [
												new sap.m.Label({ width: "150px", text: data.PayslipForm3Nav.results[i].Pyitx, textAlign : "Left" }).addStyleClass("sub-conRead-title"),
												new sap.m.Label({
													textAlign : "End",
													width: "100%",
													layoutData: new sap.m.FlexItemData({ growFactor: 1 }),
													text: data.PayslipForm3Nav.results[i].BetrgT,
												})
											]
										})	
									);
						...
						if(data.PayslipForm4Nav && data.PayslipForm4Nav.results.length > 0){
							for(var i = 0; i <data.PayslipForm4Nav.results.length; i++ ){
								if(data.PayslipForm4Nav.results[i].Pyitm != ""){
									oMoney4Layout.addItem(
										new sap.m.HBox({
											height: "40px",
											alignItems: sap.m.FlexAlignItems.Center,
											items: [
												new sap.m.Label({ width: "150px", text: data.PayslipForm4Nav.results[i].Pyitx, textAlign : "Left" }).addStyleClass("sub-conRead-title"),
												new sap.m.Label({
													textAlign : "End",
													width: "100%",
													layoutData: new sap.m.FlexItemData({ growFactor: 1 }),
													text: data.PayslipForm4Nav.results[i].BetrgT,
												})
											]
										})	
									);
			...
			oController._DetailJSonModel.setProperty("/Data/money1", common.Common.numberWithCommas(vData.money1));
			oController._DetailJSonModel.setProperty("/Data/money2", common.Common.numberWithCommas(vData.money2));
			oController._DetailJSonModel.setProperty("/Data/money3", common.Common.numberWithCommas(vData.money3));

sap.ui.define([
	...
	return CommonController.extend($.app.APP_ID, {
        ...
        changeType: function(oEvent) { // 휴/복직 구분선택
            var oController = $.app.getController();
			...
			oCodeModel.create("/CommonCodeListHeaderSet", sendObject, {
				success: function(oData, oResponse) {
					if (oData && oData.NavCommonCodeList) {
						...
						if(oController.ApplyModel.getProperty("/FormData/Status1") === "AA" || Common.checkNull(oController.ApplyModel.getProperty("/FormData/Status1")))
                   		    oUsedTypeCombo.setEditable(true);
					}
				},
				error: function(oResponse) {
					Common.log(oResponse);
        ...
        changeUsedType: function(oEvent) { // 사유 클릭시 증빙서류 안내
            var oController = $.app.getController();
			var vBukrs = oController.ApplyModel.getProperty("/FormData/Bukrs") ? oController.ApplyModel.getProperty("/FormData/Bukrs") : oController.getUserGubun();
			...
			if(oController.ApplyModel.getProperty("/FormData/Massn") === "0R"){ // 휴/복직구분의 값이 휴직인 경우
				switch(vKey) {
					case "01":  
					case "02": 
					case "03": 
					case "04":  oBabyDateBox.setVisible(false);
					...
					case "11":
					case "12":
					case "13":  oBabyDateBox.setVisible(false);
								oController.ApplyModel.setProperty("/FormData/Zexbdt", null);
								oSickInfoBox.setVisible(false);
								...
								oController.ApplyModel.setProperty("/FormData/Kdsvh", "");
								break;
					case "14":  oBabyDateBox.setVisible(true);
								oSickInfoBox.setVisible(false);
								oController.ApplyModel.setProperty("/FormData/Zdsase", "");
								oController.ApplyModel.setProperty("/FormData/Zdtopn", "");
								oFamilyInfoBox.setVisible(false);
					...
					case "18":  oBabyDateBox.setVisible(false);
								...
								break;
					default:	oBabyDateBox.setVisible(false);
								oController.ApplyModel.setProperty("/FormData/Zexbdt", null);
								oSickInfoBox.setVisible(false);
								oController.ApplyModel.setProperty("/FormData/Zdsase", "");
								oController.ApplyModel.setProperty("/FormData/Zdtopn", "");
					...
					if(vBukrs !== "A100"){
						...
						switch(vKey){
							...
							case "13":	vMsg = ele.TextA;
										...
										oController.ApplyModel.setProperty("/InfoText", vMsg.trim());
										break;
							default:	oController.ApplyModel.setProperty("/InfoText", ele.TextA);
										break;
						}
						return;
					}else{
		...
		getPartnerCheck: function() { // 배우자 육아신청여부
			var oController = $.app.getController();
			var isCombo = [];
		...
		getLeaveTerm: function(oEvent) { // 복직예정일 선택시 휴직기간
			var oController = $.app.getController();
			var oModel = $.app.getModel("ZHR_PERS_INFO_SRV");
			var vPernr = oController.getUserId();
		...
		getReinTerm: function(oEvent) { // 휴직기간 선택시 복직예정일
			var oController = $.app.getController();
		...
		getRelationOData: function() { // 관계Combo
			var oController = $.app.getController();
            var oCodeModel = $.app.getModel("ZHR_COMMON_SRV");
            var vPernr = oController.getUserId();
		...
		onLeaveReinHistory: function() { // 휴/복직 발령이력 Dialog
			var oController = $.app.getController();
			var oView = $.app.byId("ZUI5_HR_LeaveReinstatement.Page");
		...
		checkError :function(IsType) { // Error Check
			var oController = $.app.getController();
			var oFamilyInfoBox = $.app.byId(oController.PAGEID + "_FamilyInfoBox"),
                oSickInfoBox = $.app.byId(oController.PAGEID + "_SickInfoBox"),
                oBabyDateBox = $.app.byId(oController.PAGEID + "_BabyDateBox"),
		...
		onPressCancel: function(oEvent) { // 삭제
			...
			var onPressCancel = function (fVal) {
				if (fVal && fVal == oController.getBundleText("LABEL_42032")) { // 삭제
        ...
        onDialogApplyBtn: function() { // 신청
			...
			var onPressApply = function (fVal) {
				if (fVal && fVal == oController.getBundleText("LABEL_42030")) { // 신청
					...
					oModel.create("/LeaveRequestSet", sendObject, {
						success: function(oData, oResponse) {
							...
							if(Common.checkNull(!oData.Export.results[0].Url)){
								window.open(
									oData.Export.results[0].Url,
									"_blank",
        ...
        onDialogSaveBtn: function() { // 저장
			...
			var onPressSave = function (fVal) {
				if (fVal && fVal == oController.getBundleText("LABEL_42031")) { // 저장

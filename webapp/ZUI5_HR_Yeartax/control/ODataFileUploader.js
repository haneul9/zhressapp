...
**/
jQuery.sap.declare("ZUI5_HR_Yeartax.control.ODataFileUploader");
jQuery.sap.require("sap.ui.unified.FileUploader");
jQuery.sap.require("sap.m.MessageBox");

sap.ui.unified.FileUploader.extend("ZUI5_HR_Yeartax.control.ODataFileUploader", {
	metadata : {
		properties : {
			"modelName" : "string",
			"slug"	: "string"
			...
			if(files && files.length) {
				for(var i=0; i<files.length; i++) {
					...
					if (file) {
						...
						jQuery.ajax({
							type: 'POST',
							url: ($.app.getDestination() + this.getUploadUrl()),
							headers: oHeaders,
							cache: false,
